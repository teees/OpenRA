...
namespace OpenRA.Mods.Common.Activities
{
	public class Move : Activity
	{
		...
		Pair<CPos, SubCell>? PopPath(Actor self)
		{
			...
			if (!mobile.CanMoveFreelyInto(nextCell, ignoredActor, true) || containsTemporaryBlocker)
			{
				// Are we close enough?
				var cellRange = nearEnough.Length / 1024;
				if ((mobile.ToCell - destination.Value).LengthSquared <= cellRange * cellRange && !containsTemporaryBlocker)
				{
					path.Clear();
					return null;
				}
