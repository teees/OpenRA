...
{
	public class VoxelMissileInfo : IProjectileInfo
	{
		...
		public IProjectile Create(ProjectileArgs args) { return new VoxelMissile(this, args); }
	}

	// TODO: double check square roots!!!
	public class VoxelMissile : IProjectile, ISync
	{
		enum States
		{
			Freefall,
		...
		readonly VoxelMissileInfo info;
		readonly ProjectileArgs args;
		readonly VoxelAnimation anim;
		...
		public VoxelMissile(VoxelMissileInfo info, ProjectileArgs args)
		{
			this.info = info;
			this.args = args;
			...
			{
				var voxel = VoxelProvider.GetVoxel(info.Image, info.Sequences[0]); //FIXME
				anim = new VoxelAnimation(voxel, () => WVec.Zero,
					() => new[] { WRot.FromFacing(-renderFacing) },
					() => false, () => 0, true);
		...
		public IEnumerable<IRenderable> Render(WorldRenderer wr)
		{
			...
			{
				colorPalette = wr.Palette(info.Palette);
			...
			{
				yield return new VoxelRenderable(new [] {anim}, pos, 0, camera, info.Scale,
